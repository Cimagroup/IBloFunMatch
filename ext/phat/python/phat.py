...
class column(object):
    """A view on one column of data in a boundary matrix and its preimage"""
    ...
    @property
    def preimage(self):
        """The preimage values in this column, empty if cleared"""
        return self._matrix._matrix.get_preimage(self._index)
...
class boundary_matrix(object):
    ...
    def get_vector_vector(self):
        "Get boundary matrix in phat-input format"
        return self._matrix.get_vector_vector()

    def get_preimages(self):
        "Get list of preimages"
        return self._matrix.get_preimages()
