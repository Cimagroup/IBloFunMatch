...
void wrap_boundary_matrix(py::module &mod, const std::string &representation_suffix) {
  ...
  py::class_<mat>(mod, (std::string("boundary_matrix_") + representation_suffix).c_str())
    ...
    .def("get_vector_vector", [](mat &m) {
      ...
      .def("get_preimages", [](mat &m) {
          std::vector< std::vector< int > > preimage_matrix;
          m.save_preimage_matrix( preimage_matrix );
          return preimage_matrix;
        },
        "Extract the data in the preimage into a list of columns")
    ...
    .def("get_preimage", [](mat &m, phat::index col_index) {
            std::vector<phat::index> col;
            m.get_preimage(col_index, col);
            return col;
        },
        "Extract a single preimage column, cleared columns have empty preimage",
        py::arg("index"))
