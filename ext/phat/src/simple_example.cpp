...
int main( int argc, char** argv )
{
    ...
    std::cout << std::endl << "Boundary after reduction:" << std::endl;
    for( phat::index col_idx = 0; col_idx < boundary_matrix.get_num_cols(); col_idx++ ) {
        std::vector< phat::index > temp_column;
        boundary_matrix.get_col(col_idx, temp_column);
        std::cout << "Boundary column " << col_idx << " of size:" << \
            (phat::index)temp_column.size() << " values: ";
        for( phat::index idx = 0; idx < (phat::index)temp_column.size(); idx++ )
            std::cout << temp_column[ idx ] << " ";
        std::cout << std::endl;
    }

    std::cout << std::endl << "Preimage after reduction:" << std::endl;
    for( phat::index col_idx = 0; col_idx < boundary_matrix.get_num_cols(); col_idx++ ) {
        std::vector< phat::index > temp_column;
        boundary_matrix.get_preimage(col_idx, temp_column);
        std::cout << "Preimage column " << col_idx << " of size:" << \
            (phat::index)temp_column.size() << " values: ";
        for( phat::index idx = 0; idx < (phat::index)temp_column.size(); idx++ )
            std::cout << temp_column[ idx ] << " ";
        std::cout << std::endl;
