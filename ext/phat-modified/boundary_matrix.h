...
// interface class for the main data structure -- implementations of the interface can be found in ./representations
 ...
 template< class Representation = bit_tree_pivot_column >
   class boundary_matrix
    ...
    protected:
        Representation rep;
        Representation preimage_matrix;
    ...
    public:
        ...
        // but some do no
        void set_dimensions( index nr_of_rows, index nr_of_columns ) {
            rep._set_dimensions( nr_of_rows, nr_of_columns );
            init_preimage();
        }

        // set overall number of columns in boundary_matrix
        // sets the number of rows to nr_of_columns as well!
        void set_num_cols( index nr_of_columns ) {
            rep._set_dimensions( nr_of_columns, nr_of_columns );
            init_preimage();
        }
        ...
        // set column @idx to the values contained in @col
        void set_preimage( index idx, const column& preimage  ) {
      	       preimage_matrix._set_col( idx, preimage ); }

        // replaces content of @col with preimage of given index
        void get_preimage( index idx, column& col ) const {
            preimage_matrix._get_col( idx, col );
        }
        ...
        void add_to( index source, index target ) {
            ...
            preimage_matrix._add_to( source, target );
        }

        // clears given column
        void clear( index idx ) {
	          rep._clear(idx);
            // preimage is cleared as it can be read from boundary matrix
            preimage_matrix._clear(idx);
        }
    ...
    public:
        ...
        void init_preimage() {
            /* Initializes the preimage matrix*/
            index no_cols = get_num_cols();
            preimage_matrix._set_dimensions( 1, no_cols );
            for( index i = 0; i < no_cols; i++){
                std::vector< phat::index > temp_col(1,i);
                preimage_matrix._set_col(i, temp_col);
            }
        }
    ...
    public:
        ...
        void save_preimage_matrix( std::vector< std::vector< index_type > >& output_matrix ) {
            const index nr_of_columns = get_num_cols();
            output_matrix.resize( nr_of_columns );
            column temp_col;
            for( index cur_col = 0; cur_col <  nr_of_columns; cur_col++ ) {
                get_preimage( cur_col, temp_col );
                index num_rows = temp_col.size();
                output_matrix[ cur_col ].clear();
                output_matrix[ cur_col ].resize( num_rows );
                for( index cur_row = 0; cur_row <  num_rows; cur_row++ )
